...
def processjobs(config, opts, procid):
    ...
    if 'profile' in opts and opts['profile'] == True:
        ...
        title = "| {0:<23} {1:<16} {6:<4} {2:<5}  {3:<16} {6:<4} {2:<5}  {4:<17} {6:<4} {2:<5}  {5:<17} {6:<4} {2:<5} |".format("analytic/preproc", 'total (sec)', '%', 'extract (sec)', 'process (sec)', 'results (sec)','freq')
        logging.info('-'*len(title))
        logging.info(title)
        for k, val in p.ranked():
            if 'total' in val:
                if 'process' in val and 'results' in val:
                    logging.info("| {0:<23} {1:<16} {2:<4} {3:05.2f}  {4:<16} {5:<4} {10:05.2f}  {6:<17} {7:<4} {11:05.2f}  {8:<17} {9:<4} {12:05.2f} |".format(k, val['total'].sum(), val['total']._count, (val['total'].sum() / del_t)*100, val['extract'].sum(), val['extract']._count, val['process'].sum(), val['process']._count, val['results'].sum(), val['results']._count, (val['extract'].sum()/del_t)*100, (val['process'].sum()/del_t)*100, (val['results'].sum()/del_t)*100))
                else:
                    toprint = "| {0:<23} {1:<16} {2:<4} {3:05.2f}".format(k, val['total'].sum(), val['total']._count, (val['total'].sum() / del_t)*100)
                    toprint = toprint + (" "*(len(title) - len(toprint)-1)) + '|'
                    logging.info(toprint)
            else:
                toprint = "| {0:<23} {1:<17}".format(k, val)
                toprint = toprint + (" "*(len(title) - len(toprint)-1)) + '|'
                logging.info(toprint)
        logging.info('-'*len(title))
